...
export class TournamentParticipantsComponent implements OnInit, OnDestroy, AfterViewInit, OnChanges {
        ...
        this.userService.searchUsers(searchTerm).pipe(
            ...
            next: (users: User[]) => {
                ...
                this.searchResults = this.searchResults.filter(user => 
                    !participantUids.includes(user.firebaseUid)
                );
    ...
    addParticipant(player: UserProfile): void {
        ...
        this.userService.getUserByFirebaseUid(player.firebaseUid).pipe(
            takeUntil(this.destroy$)
        ).subscribe({
            next: (userFromDb: any) => {
                ...
                this.tournamentService.addTournamentParticipant(tournamentId, participant, this.currentUser?.firebaseUid || '').pipe(
                    takeUntil(this.destroy$)
                ).subscribe({
                    next: () => {
                        this.messageService.add({
