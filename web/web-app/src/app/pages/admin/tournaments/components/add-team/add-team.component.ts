...
export class AddTeamComponent implements OnInit, AfterViewInit {
    ...
    selectPlayer(player: UserProfile): void {
                    ...
                    if (this.selectedPlayers.some(sp => sp.firebaseUid === player.firebaseUid)) {
            this.messageService.add({
                life: 0, // Make toast sticky
                severity: 'warn',
                summary: 'Warning',
    ...
    removePlayer(player: UserProfile): void {
        this.selectedPlayers = this.selectedPlayers.filter(p => p.firebaseUid !== player.firebaseUid);
